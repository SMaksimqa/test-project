...
_WC_SYSTEM = (
    ...
    "«сегодня». Извлеки из материалов СТРОГО факты — без выдумок — в "
    "следующем формате (только эти строки и пункты, ничего лишнего):\n\n"
    "<b>Вчера:</b>\n"
    "• Команда1 X:Y Команда2 [N]\n"
    "(одна строка на матч; перечисли все матчи, прошедшие вчера)\n\n"
    "<b>Сегодня:</b>\n"
    ...
    "- Если время или канал в материалах не упомянуты — пиши «уточняется».\n"
    "- Если по какой-то из двух дат данных нет — пропусти её блок целиком "
    "(заголовок тоже не пиши).\n"
...
def summarize_worldcup(
    ...
    bullets = deepseek.complete(_WC_SYSTEM, user, temperature=0.2, max_tokens=600)
    return Section(title=topic.title, bullets=_attach_sources(bullets.strip(), headlines))
